...
public:
    ...
    static zone_type make(const lower_bound_type& bmin, const upper_bound_type& bmax,const lower_bound_type& emin,const upper_bound_type& emax, const lower_bound_type& dmin, const upper_bound_type& dmax){
        ...
        return zone_type(
            lower_bound_type::intersect(
                bmin, 
                lower_bound_type::add(emin, dmax)),

            upper_bound_type::intersect(
                bmax, 
    ...
    static zone_type make(const std::array<T, 6>& values, const std::array<bool, 6>& signs){
        return make(lower_bound_type(values[0], signs[0]),
    ...
    static zone_type make_from_interval_no_anchor(value_type begin, value_type end){
        return make({begin, end, begin, end, 0, end-begin});
    }

    static zone_type make_from_interval_begin_anchored(value_type begin, value_type end){
        return make({begin, begin, begin, end, 0, end-begin}, {1,1,1,1,0,1});
    }

    static zone_type make_from_interval_end_anchored(value_type begin, value_type end){
        return make({begin, end, end, end, 0, end-begin}, {1,1,1,1,0,1});
    }

    static zone_type make_from_interval_dual_anchored(value_type begin, value_type end){
    ...
    static zone_type concatenate(const zone_type& z1, const zone_type& z2){
    ...
    static zone_type intersect(const zone_type& z1, const zone_type& z2){

        return make(lower_bound_type::intersect(z1.bmin, z2.bmin),
                    upper_bound_type::intersect(z1.bmax, z2.bmax),
                    lower_bound_type::intersect(z1.emin, z2.emin),
                    upper_bound_type::intersect(z1.emax, z2.emax),
                    lower_bound_type::intersect(z1.dmin, z2.dmin),
                    upper_bound_type::intersect(z1.dmax, z2.dmax));

    }


    static zone_type constrain(const zone_type& z, const lower_bound_type& dmin1, const upper_bound_type& dmax1){
        ...
        return make(
            ...
            z.get_emin(),
            z.get_emax(),
            lower_bound_type::intersect(z.get_dmin(), dmin1),
            upper_bound_type::intersect(z.get_dmax(), dmax1)
        );

    }

    static zone_type constrain(const zone_type& z, const value_type dmin1, const value_type dmax1){
        return constrain(z, 
    ...
    static zone_type constrain_end(const zone_type& z, const lower_bound_type& emin1, const upper_bound_type& emax1){

        return make(
            z.get_bmin(),
            z.get_bmax(),
            lower_bound_type::intersect(z.get_emin(), emin1),
            upper_bound_type::intersect(z.get_emax(), emax1),
            z.get_dmin(),
            z.get_dmax()
        );
    ...
    static zone_type constrain_end(const zone_type& z, const value_type emin1, const value_type emax1){
        return constrain_end(z, 
            lower_bound_type::open(emin1), 
            upper_bound_type::closed(emax1)
        );
